...
def test_web_profile_is_fixed_and_read_only(client, tmp_path):
    """The browser controller uses the packaged controls.web.yaml; it cannot be saved or overridden."""
    web = profile(client, 'web')
    uart = profile(client, 'uart')
    assert web['controller'] == 'web'
    assert web['editable'] is False and uart['editable'] is True
    ...
    # Speeds and acceleration are the UART values.
    ...
    assert '変更できません' in response.json()['detail']
    assert not (tmp_path / 'controls.web.yaml').exists()
    # A file of the same name in the config directory is never used for the web profile.
    (tmp_path / 'controls.web.yaml').write_text('bad: [')
    assert profile(client, 'web')['values'] == web['values']


def test_readiness_accepts_web_controller(client, monkeypatch, tmp_path):
    """CONTROLLER_TYPE=web validates the packaged controls.web.yaml, not the UART file."""
    ...
    # A stray saved web file cannot break it: the packaged profile is always used.
    (tmp_path / 'controls.web.yaml').write_text('bad: [')
    assert client.get('/api/readiness').json()['profile']['ok']
