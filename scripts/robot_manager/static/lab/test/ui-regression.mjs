...
// Runs inside the page before any site script: repeatable time and randomness.
const DETERMINISM = `(() => {
  let now = 1000, seed = 12345, frame = 0; const queue = new Map();
  ...
  // The harness pumps frames explicitly, 16 ms of fake time each.
  window.__pumpFrames = (count) => { for (let i = 0; i < count; i++) { now += 16; const batch = [...queue]; queue.clear(); for (const [, fn] of batch) fn(now); } };
})();`;
...
async function runSite(origin, route, steps) {
  ...
  try {
    ...
    const click = (scope, index) =>
      browser.evaluate(
        `(() => { const el = [...document.querySelectorAll(${JSON.stringify(scope)})][${index}]; if (!el || el.disabled) return false; el.click(); return true; })()`,
      );
    if (steps) {
...
function report(name, before, after, maxLines) {
  if (before === after) return true;
  const a = before.split('\n');
  const b = after.split('\n');
  console.log(`DIFF  ${name}`);
  let shown = 0;
  for (let i = 0; i < Math.max(a.length, b.length) && shown < maxLines; i++) {
    if (a[i] === b[i]) continue;
    console.log(
...
try {
  ...
  for (let i = 0; i < Math.min(baseline.snapshots.length, candidate.snapshots.length); i++) {
    if (
      report(
  ...
  console.log(
    `${args.route}: ${same}/${baseline.snapshots.length} snapshots identical` +
