...
「速度・操作感」は日常の調整に使う4項目に絞っています。
走行速度は m/s、旋回速度は度/秒、ローラー出力・スティックの遊びは % で表示します。
表示単位は保存時に ROS の単位へ変換し、既存の走行・旋回方向の反転も維持します。
各項目に数値を変えたときの効果を説明し、「保存済み」と「変更後」を並べています。
速度は指令値で、実際の速度は車体の制限や路面によって変わります。

加速度、モータ RPM、低速不感帯、目標速度付近の緩和幅、全方向移動用の左右速度、
単体起動専用の設定は画面に表示しません。保存時にも既存値を維持します。
「速度・操作感を初期値に戻す」は表示中の4項目だけを対象とし、キー割り当ても維持します。
