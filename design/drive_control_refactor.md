...
1. ~~`stop_resend_interval_ms` の 0 / 200 A/B~~ → **解決（実機）**: `brake_on_stop: false` + `stop_resend_interval_ms: 300` が最良。実機で報告された「2段階停止」（減速したあと最後だけ挙動が変わる）の主因は停止フレームの再送頻度だった。制御 tick 導入で停止再送が 50 Hz になったことで顕在化したもの。
   - 事前の仮説（`brake_on_stop: true` にすれば能動的な 0 保持で解決する）は**外れ**。ブレーキの有無ではなく再送のスロットルが効いた。停止フレームは毎回ファーム速度ループへの新しい目標指令として作用するため、高頻度で送るとファーム側が自身の減速を完了できないと考えられる（ファーム内部は観測できないため機序は推定のまま）。
   - この機序はホスト側のシミュレーションでは再現できない（プラントモデルは指令頻度に対する応答を模していない）。**ファームの指令頻度依存性は実機でしか分からない領域**として記録しておく。
...
4. ~~デマンド適応の縮退可否（Phase 3）~~ → **解決（実機 + シミュレーション）**: デマンド適応（`min_*_accel`）は無効化した。残差ベースの適応は「入力の丁寧さ」と「追従の遅れ」を区別できず、狙い（ゆっくり倒したとき穏やか）と逆に**微小入力の応答を鈍くしていた**（0.3 rad/s の到達が 0.52 秒 → 適応無効で 0.14 秒）。実機でも「追従性が低い」として報告された。ジャーク抑制はテーパー（`slew_taper_band_*`）が担うため機能は失われない。P3 の「パラメータが直交していない」の具体例として、機構を 1 つ減らせた。
   - フィーリング要件として「ゆっくり倒したら穏やか」が本当に必要になった場合は、残差ベースではなく**入力の微分ベース**（スティックを速く倒したかどうか）の整形に置き換えること。残差では上記の混同が避けられない。

5. 旋回の追従の速さ（`max_angular_accel`）。整定 2.04 秒のうち 1.82 秒はレート上限そのもので、上げれば比例して速くなる（6.0 で 1.02 秒）。ただし低 RPM 域のファームループ励起を抑える目的で低く設定された経緯（PR #141）があるため、実機で段階的に上げて振動が出ない上限を同定する必要がある。実行時変更に対応済み（`ros2 param set /drive_component max_angular_accel 4.5`）。
