...
- 実行時再設定時のリセット規則: `velocity_run_*`（`velocity_run_feedback_max_age_sec` を除く）を実行時に変更したら、旧モデルで育ったオブザーバ・LQR の内部状態を破棄し、次の有効なフィードバックで実測 RPM から初期化し直す。スルーレートの前回指令と `DriveMode` は維持する（PR #144 の「走行中にパラメータを変えても指令が飛ばない」性質）。`velocity_run_*` 以外の変更では車輪制御器の状態を消さない。RUN 閾値（`drive_fsm_run_*`）の変更は、モードが移れば既存の遷移処理が同じリセットを行う。
