...
- `questix_lab_bridge/`: WebSocket bridge (ament_python) that mirrors `/scan`, `/odom`, `/drive_status`, `/target_twist`, and an optional camera topic to QUESTiX LAB. Observation only unless `allow_drive` is set (robot_manager 教材 tab; off by default, in competition mode and again after every robot_manager restart); then it may publish `/target_twist` and nothing else, under the checks in `questix_lab_bridge/drive.py` (no other publisher, drive node present, E-stop released, one page, speed limits, dead-man timeout). Do not add other publishers, services or actions, and do not weaken those checks.
