...
- `questix_lab_bridge/`: WebSocket bridge (ament_python) that mirrors `/scan`, `/odom`, `/drive_status`, `/target_twist`, and an optional camera topic to QUESTiX LAB. Observation only unless `allow_drive` is set (robot_manager 教材 tab; on in practice mode, off in competition mode); then it may publish `/target_twist/lab` (twist_arbiter's lab input) and nothing else, under the checks in `questix_lab_bridge/drive.py` (no other publisher, arbiter present, E-stop released, one page, speed limits, dead-man timeout, controller takeover). Do not add other publishers, services or actions, and do not weaken those checks.
