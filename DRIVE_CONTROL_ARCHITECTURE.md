...
- ~~シリアル帯域: baud を上げられるか~~ → **解決済み: バスは変更不可（57600 固定）**。代わりに測るべきはファーム応答遅延 `T_fw`（write 完了 → 応答完了の分布）で、これがフィードバックタイムアウトの最適値を決める。§4「シリアルバス制約が決めるレート設計」参照。
...
flowchart TD
  ...
  E["drive_controller ★新規<br/>固定周期 50 Hz"] -->|/wheel_cmd| F
...
**制御層 `drive_controller`（新規、これが `control_module`）** — **固定周期（50 Hz、バス周期に一致）で回るループ**。中身は ROS もシリアルも知らない純 C++ クラスとして実装する:
...
### シリアルバス制約が決めるレート設計（バスは変更不可）

**前提: シリアルバスは変えられない。** 57600 baud、1本のバスで左右モータを request-response、10 バイトフレーム — これは固定条件として扱う。baud 引き上げは選択肢から外す。レート設計はこの帯域から計算で決まる。

素の帯域:

| 項目 | 値 |
|---|---|
| 57600 baud 8N1 | 5,760 B/s → **0.174 ms/byte** |
| 10 バイトフレーム | 1.74 ms |
| 指令 + フィードバック往復（1モータ） | 3.47 ms + ファーム応答遅延 `T_fw` |
| 2 モータ = 1 制御周期の最小コスト | **6.94 ms + 2·T_fw** |

理論上の上限は約 144 Hz（バス占有 100 %）だが、実際に効くのは平均帯域ではなく**フィードバック取りこぼし時のタイムアウト**である。現在の `readFeedbackFrame` のタイムアウトは 10 ms なので、1 モータが取りこぼすと `1.74 + 10 = 11.7 ms` を消費する。

`T_fw ≈ 1 ms` と仮置きした worst-case 予算（要 Step 0 実測）:

| 制御周期 | 両方成功 | 片方取りこぼし | 両方取りこぼし |
|---|---|---|---|
| 100 Hz（10 ms） | 8.9 ms — 89 % | 16.2 ms — **超過** | 23.5 ms — **超過** |
| 50 Hz（20 ms）・timeout 10 ms | 8.9 ms — 45 % | 16.2 ms — 81 % | 23.5 ms — **超過** |
| 50 Hz（20 ms）・timeout 5 ms | 8.9 ms — 45 % | 11.2 ms — 56 % | 13.5 ms — 67 % |

ここから3つ決まる。

**決定 1: 制御周期はバス周期に合わせて 50 Hz にする。** 以前「制御 100 Hz / バス 50 Hz」と書いたが撤回する。100 Hz では**フィードバックを1回落とすだけで周期を超過する**。また、実際に指令を打てるのが 50 Hz なら、100 Hz で計算しても半分は捨てるだけで実効的な制御権限は増えず、「どのサンプルが実際に送られたのか」が曖昧になる。1つのレートに揃えるほうが推論もチューニングも簡単である。将来ジャーク分解能を上げたくなったら、`step(dt, ...)` の契約はレート変更を許すので後から上げられる。

**決定 2: フィードバックタイムアウトを実測から決める。** 10 ms は往復の実測（3.47 ms + `T_fw`）に対して過剰に緩く、しかも worst-case 予算を壊している唯一の要因である。**Step 0 で write 完了 → 応答完了の分布を測り、p99.9 から決める**（4〜5 ms が狙い）。ここは「バスを変えずに worst-case を 67 % に収める」唯一のレバーなので、実測項目として優先度が高い。

**決定 3: 観測は指令のついでに取り、追加のバス往復をゼロにする。** 現状は `statusTimerCallback`（50 Hz）が `refreshMotorFeedback` で**別途フレームを再送**しており、指令経路と観測経路がバスを二重に食っている。バスが固定なら、これは払えない。

`ddt_bus_driver`（Step 1）がバス周期を所有すれば、**指令フレームの応答がそのままフィードバックになる**ので、観測は追加コストなしで 50 Hz で得られる。`refreshMotorFeedback` は消える。副次的に、観測のサンプリング間隔が厳密に一定になるので、実測 RPM ローパスとオドメトリ積分の `dt` も定数になる（P7 の解消）。

- ただし停止中は指令が「停止」なので、`stop_resend_interval_ms` でブレーキ再送を絞ると**その間フィードバックが止まる**。停止中の実測が古くなるのは許容する（現行コードのコメントも同じ判断）。トレードオフが 1 箇所に明示される形になる。
- `status_publish_rate` は publish のレートであってバスのレートではない。キャッシュした最新状態を publish するだけなので、バス帯域とは無関係になる。

**決定 4: 閉ループの帯域上限もここで決まる。** フィードバックが 50 Hz（20 ms サンプル）なら、位相余裕を確保した実用的な閉ループ帯域はサンプルレートの 1/10 程度、すなわち **約 5 Hz が上限**である。プラントの振動は 1.8 Hz。両者が 3 倍程度しか離れていない。

- **Plan V（ファーム速度ループの上にカスケード）**: 外側ループは内側より十分遅くする必要があるが、内側は 1.8 Hz で振動している。外側を 1.8 Hz より十分下（時定数 ~1 s）に置くしかなく、つまり **Plan V は定常追従誤差は直せても 1.8 Hz の振動は抑えられない**。
- **Plan C（電流モードでホストが速度ループを持つ）**: ファーム速度ループを使わないので、1.8 Hz の振動は抑制するのではなく**構造的に消える**。代わりにホストの速度ループが主ループになり、50 Hz サンプルで 3〜5 Hz の帯域を取る必要がある。車輪の機械時定数が 100 ms 程度なら 1 時定数あたり約 5 サンプルで、成立はするが余裕は少ない。

つまり**バス固定という制約は Plan C の相対的な魅力を上げる**（振動を抑え込むのではなく消せるため）が、同時に 50 Hz サンプリングという厳しい条件を課す。機械時定数の実測（Step 0 のステップ応答）が判断材料になる。
...
- **ファーム応答遅延 `T_fw` の分布**（write 完了 → 応答フレーム完了までの時間）。バスが固定なのでフィードバックタイムアウトが worst-case 予算を決める唯一のレバーになる。p99.9 からタイムアウト値を決める（§4 決定 2）
- **フィードバック取りこぼし率**と、その連続発生の有無（両輪同時に落ちる頻度が予算超過の条件）
- **車輪の機械時定数**（ステップ応答の立ち上がり）。50 Hz サンプルで閉ループが成立するかの判断材料（§4 決定 4）
...
2. **制御タイマー（50 Hz = バス周期）が唯一の制御ステップ**。新しい指令が来ていなくても毎周期回す（ランプの継続、ウォッチドッグ、停止フレームの再送はすべて時間駆動であって指令駆動ではない）。
...
6. **レートをパラメータとして分離する**。制御ループとバス周期は 50 Hz で一致させるが（§4 決定 1）、ステータス・オドメトリ publish は独立したパラメータにする。publish はキャッシュ済み状態を出すだけでバス帯域を消費しない。現状はステータスタイマーが指令とバスを取り合っていて絡んでいる。
...
- 閉ループゲインの最終調整（Step 4）
- ファーム応答遅延 `T_fw` の分布 → フィードバックタイムアウトの決定（§4 決定 2）
...
- バスが固定（57600、変更不可）なので帯域に伸びしろがない。worst-case 予算を守るレバーはフィードバックタイムアウトのみ（§4 決定 2）。両輪同時取りこぼしで周期超過しないことを実測で確認する
...
3. ~~シリアル baud を上げられるか~~ → **解決済み: バスは変更不可**。レート設計は §4 に反映済み。残る論点は、閉ループ帯域上限 約 5 Hz とプラント振動 1.8 Hz が 3 倍しか離れていない点をどう扱うか（Plan V では抑えられない → Plan C 寄りになる）。
